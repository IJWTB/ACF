	...
	if ent:IsValid() then
		if ent:IsPlayer() || ent:IsNPC() then
			...
			if ( Valid and ent.ACF.Health < ent.ACF.MaxHealth ) then
				...
				ent:EmitSound( "ambient/energy/NewSpark0" ..tostring( math.random( 3, 5 ) ).. ".wav" )--Welding noise here, gotte figure out how to do a looped sound.
	...
	if ent:IsValid() then
		...
		if Valid then
			...
			if HitRes.Kill then
				...
				ent:EmitSound( "weapons/physcannon/superphys_small_zap" ..tostring( math.random( 1, 4 ) ).. ".wav" ) --old annoyinly loud sounds
			end
		else 
			self.Weapon:SetNWFloat( "HP", 0 )
			self.Weapon:SetNWFloat( "Armour", 0 )
